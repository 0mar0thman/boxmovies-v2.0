...
function updateSliderPositionVoteCount() {
    ...
    sliderVoteCount.style.transform = `translateX(-${currentCardIndexVoteCount * (cardWidth + 10)}px)`;
}

document.addEventListener('click', (event) => {
    if (event.target.closest('.btn-card-vote-count')) {
        const index = event.target.closest('.btn-card-vote-count').getAttribute('data-index');
...
document.getElementById('nextCardButtonVoteCount').addEventListener('click', () => {
...
document.getElementById('prevCardButtonVoteCount').addEventListener('click', () => {
...
function updatePaginationVoteCount(currentPageVoteCount, totalPagesVoteCount) {
    ...
    const maxButtons = 5;
    const startPage = Math.max(1, currentPageVoteCount - Math.floor(maxButtons / 2));
